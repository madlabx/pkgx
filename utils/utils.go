...
func HttpPostBody(url string, body interface{}, timeout int) (*http.Response, []byte, error) {
	...
	if err != nil {
		...
		return nil, nil, base.NewStatusError(http.StatusBadRequest, base.ECODE_BAD_REQUEST_PARAM, err)
	}
...
func HttpPost(url string, body interface{}, timeout int) (*http.Response, error) {
	...
	if err != nil {
		log.Errorf("Parse json failed, url: %s, obj: %#v", url, body)
		return nil, base.NewStatusError(http.StatusBadRequest, base.ECODE_BAD_REQUEST_PARAM, err)
	}
...
func requestBytesForBody(method, requrl string, bodyBytes []byte, timeout int, wantBody bool) (*http.Response, []byte, error) {
	...
	if err != nil {
		...
		return nil, nil, base.UnWrapperError(err)
	}
	defer func() {
		if rsp != nil {
			rsp.Body.Close()
...
func requestBytesForBodyNormal(method, requrl string, bodyBytes []byte, wantBody bool) (*http.Response, []byte, error) {
	...
	if rsp.StatusCode != http.StatusOK && rsp.StatusCode != http.StatusCreated {
		...
		if err != nil {
			log.Errorf("read body err, err:%v, response:%v", err.Error(), rsp)
			return nil, nil, base.NewStatusErrStr(rsp.StatusCode, base.ECODE_FAILED_HTTP_REQUEST, "Failed to parse response body")
		}

		//log.Errorf("Got not 200 response[%#v], body[%v]", rsp, string(body))

		newStatusError := new(base.StatusError)
		err = json.Unmarshal(body, newStatusError)
		if err != nil {
			log.Errorf("read body err, err:%v, response:%v", err.Error(), rsp)
			return nil, nil, base.NewStatusErrStr(rsp.StatusCode, base.ECODE_FAILED_HTTP_REQUEST, "Failed to parse error information: "+string(body))
		}
		newStatusError.Status = rsp.StatusCode

		if newStatusError.Code == base.ECODE_SUCCESS {
			newStatusError.Code = base.ECODE_FAILED_HTTP_REQUEST
